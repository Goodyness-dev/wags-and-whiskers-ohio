...
export default function AdminLayout({ user, onLogout, onBackToSite }) {
  ...
  const navItems = [
    ...
    { id: 'orders', label: 'Care Requests & Bookings', icon: ClipboardList, badge: stats.total > 0 ? stats.total : null },
    { id: 'inbox', label: 'Client Messages', icon: MessageSquare, badge: stats.pending > 0 ? stats.pending : null },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f8] text-slate-900 font-sans flex antialiased">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200/80 flex flex-col justify-between transition-transform duration-300 ease-in-out ${
        ...
        <div className="p-6 space-y-8 flex-1 overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 min-w-0">
              <div className="w-10 h-10 shrink-0 rounded-2xl bg-[#1E3D2F] text-white flex items-center justify-center shadow-md shadow-[#1E3D2F]/30">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3.5 py-3 rounded-2xl text-xs sm:text-sm font-bold transition ${
                      isActive
                        ? 'bg-[#1E3D2F] text-white shadow-md shadow-[#1E3D2F]/25'
                    ...
                    {item.badge && (
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                        isActive
                          ...
                          : 'bg-[#E8F0EA] text-[#1E3D2F] border border-[#E2EAE4]'
                      }`}>
                        {item.badge}
                      </span>
                    )}
        ...
        {/* Bottom Banner Card */}
        <div className="p-4 m-4 rounded-2xl bg-gradient-to-br from-shop-red to-[#152C22] text-white space-y-2 shadow-lg shadow-[#1E3D2F]/20">
        ...
        <header className="h-20 bg-white border-b border-slate-200/80 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
          ...
          <div className="flex items-center space-x-3 flex-1 max-w-md">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-[#f8fafc] border border-slate-200 focus:border-[#1E3D2F] focus:bg-white rounded-2xl pl-10 pr-12 py-2 text-xs text-slate-800 placeholder-slate-400 outline-none transition"
              />
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            >
              ...
              {stats.pending > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#1E3D2F] text-white text-[10px] font-bold flex items-center justify-center shadow-xs">
                  {stats.pending}
                </span>
              )}
            </button>
            ...
            >
              ...
              <span className="absolute 2.5 2.5 w-2 h-2 rounded-full bg-[#1E3D2F]" />
            </button>

            {/* Admin Profile Card */}
            <div className="flex items-center space-x-3 pl-2 border-l border-slate-200">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-shop-red to-[#152C22] text-white font-black text-sm flex items-center justify-center shadow-sm">
              ...
              <div className="hidden sm:block text-left">
                <h4 className="text-xs font-black text-slate-900 leading-tight truncate max-w-[130px]">
                  {BUSINESS_INFO.owner?.name || 'Care Admin'}
                </h4>
