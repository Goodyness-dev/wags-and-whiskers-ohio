    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white border border-emerald-200/80 rounded-2xl p-5 shadow-xs hover:shadow-md transition">
          ...
          <span className="text-[11px] text-emerald-600/80 mt-1 block font-medium">Pet(s) serviced & closed</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-5 space-y-4 shadow-xs">
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-1 text-xs scrollbar-none">
          {[
            ...
            { id: 'pending', label: 'â³ Needs Quote', count: stats.pending },
            { id: 'quoted', label: 'ðŸ“§ Quoted', count: stats.quoted },
            { id: 'completed', label: 'âœ… Completed', count: stats.completed },
            { id: 'archived', label: 'ðŸ“¦ Archived' }
          ].map(tab => (
            <button
              key={tab.id}
            ...
            <table className="w-full text-left text-xs text-slate-700">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-bold border-b border-slate-200 text-[11px]">
                <tr>
                  ...
                  <th className="py-3.5 px-4">Pet(s)</th>
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border ${
                          ...
                          {status === 'pending' ? 'â³ Pending' :
                           status === 'quoted' ? 'ðŸ“§ Quoted' :
                           status === 'completed' ? 'âœ… Completed' : 'ðŸ“¦ Archived'}
                        </span>
                      </td>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs text-slate-600">
                    <strong className="text-slate-900">{q.make}</strong> â€¢ {q.modelAndYear}
                  </div>
